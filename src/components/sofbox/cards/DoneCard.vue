<template>
  <iq-card class-name="dash-hover-gradient1 iq-card-card done"
           header-bg-color="transparent"
           header-class="d-flex justify-content-between border-0"
           body-class="body-scroll"
  >
    <template v-slot:headerTitle>
      <div class="mb-0 card-item-position done-title-design">
        <i class="ri-checkbox-fill font-weight-light font-size-32 "  style="color: darkgoldenrod"></i>
    ...
    <template v-slot:body >
      <draggable v-model="done" group="people" @change="log">
        <div v-for="(task, index) in done" :key="index" class="notepad-body" >
          <div style="width: 100%">
      ...
      <div class="text-center" v-if="doneError"><span>{{doneError}}</span></div>
    </template>
...
export default {
  ...
  created() {
    store.dispatch('TaskData/getTaskDone')
  },
  computed: {
    done:{
      get:function (){
        return store.getters["TaskData/done"]
      },
      set:function (newValue){
        store.commit('TaskData/done', newValue)
      }
    },
    currentLink(){
      return store.getters["TaskData/doneCurrentLink"]
    },
    pagination(){
      return store.getters["TaskData/donePagination"]
    },
    doneError(){
      return store.getters["TaskData/doneError"]
    },
    userId(){
    ...
    handleScrolledToBottom(isVisible) {
      if (isVisible){
        if(this.pagination && this.pagination[this.currentLink+1] && this.pagination[this.currentLink+1].label !== 'Next &raquo;'){
          store.dispatch("TaskData/getPaginateTaskDone",this.pagination[this.currentLink+1].url)
        }
      }
